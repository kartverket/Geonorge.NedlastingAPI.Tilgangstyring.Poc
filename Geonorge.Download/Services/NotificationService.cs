...
namespace Geonorge.Download.Services
{
    public class NotificationService(ILogger<NotificationService> logger, IConfiguration config, IEmailService emailService) : INotificationService
    {
        ...
        public MailMessage CreateReadyForDownloadBundleEmailMessage(Order order)
        {
            ...
            var downLoadApiUrl = new DownloadUrlBuilder(config).OrderId(order.Uuid).AsBundle();
        ...
        private MailMessage CreateEmail(string email)
        {
            ...
            message.From = new MailAddress(config["WebmasterEmail"]);
        ...
        public MailMessage CreateReadyForDownloadEmailMessage(OrderItem orderItem)
        {
            ...
            { 
                ...
                if (!string.IsNullOrEmpty(item.DownloadUrl))
                {
                    ...
                    var downLoadApiUrl = new DownloadUrlBuilder(config).OrderId(item.Order.Uuid).FileId(item.Uuid)
                        .Build();
                    body.AppendLine(downLoadApiUrl);
                    AddPrivacyInfo(body);
                }
        ...
        private MailMessage CreateOrderStatusNotDeliverableEmailMessage(Order order)
        {
            var email = order.email;
            var message = CreateEmail(email);
            message.Bcc.Add(new MailAddress(config["SupportEmail"]));
